...
DiffusionRate_Values = [0,logspace(-10,10,50)];   %rate of diffusion for proteins (diffusion events/time unit)
...
wait = waitbar(0,'0/51 Simulations Complete');

for Trial = 1:numel(DiffusionRate_Values)
    Diffusion_Rate(Trial) = DiffusionRate_Values(Trial);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if Event >= minIterations %only tests for equilibrium after 100 events
            ...
            if abs(CurveFit(1)) < 0.01 & (Y_Int_Error(Event) < 0.05 | isnan(Y_Int_Error(Event)))  %if fitted slope is basically zero and the fitted y-int is basically the average saturation...
    ...
    % Error = scatter(t(101:end),Y_Int_Error(100:end),5,'b','filled');
    % hold on;
    % Criteria = yline(0.01,'--b','Equilibrium Criteria: 0.01','LabelHorizontalAlignment','center');
    % yyaxis right; ylabel('Saturation Level'); ylim([0 1]);
    % Profile = scatter(t,FracCover,2,'r','filled');
    % Eq = yline(Equilibrium_Saturation,'--r',['Eq. Saturation: ', num2str(round(Equilibrium_Saturation,2))],'LabelHorizontalAlignment','center');
    % Eq_t = xline(Equilibrium_t,'k',['Eq. Time: ', num2str(round(Equilibrium_t,2))],'LabelVerticalAlignment','top');
    % legend([Error,Profile],'Y-Intercept Error','Saturation','Location','North');
    ...
    waitbar(Trial/numel(DiffusionRate_Values),wait, [num2str(Trial), '/51 Simulations Complete']);
end
close(wait);
...
Sat = semilogx(DiffusionRate_Values,Equilibrium_Saturation,'o','MarkerFaceColor','r','MarkerEdgeColor','none','MarkerSize',5);    hold on;
yline(Equilibrium_Saturation(DiffusionRate_Values == 0),'--r','Zero Diffusion','LabelHorizontalAlignment','center');
xlabel('Diffusion Rate (Events/Time Interval)');    xlim([0 max(DiffusionRate_Values)]);
yyaxis right;   ylabel('Equilibrium Time'); ylim([0 max(Equilibrium_t)+0.25*max(Equilibrium_t)]);   hold on;
Time = semilogx(DiffusionRate_Values,Equilibrium_t,'o','MarkerFaceColor','b','MarkerEdgeColor','none','MarkerSize',5); hold on;
